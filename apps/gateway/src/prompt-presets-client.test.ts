...
describe('Prompt Presets loopback client', () => {
  ...
  it('returns sanitized profile metadata and never exposes prompt content', async () => {
    ...
    const fetchImpl = vi.fn(async (url: string | URL | Request) => {
      ...
      profile: {
        ...
        entries: [{
          ...
          tags: ['rp'], renderOnly: true, content: 'PROMPT_CANARY_SECRET', source: { sourcePath: 'C:/secret' },
        }],
      },
      })
    }) as unknown as typeof fetch
    const client = createPromptPresetsClient({ fetchImpl })
    const profile = await client.profile('dreamwhale-v3-agent', 1)

    expect(requestedUrls).toEqual(['http://127.0.0.1:3091/prompt-presets/api/profiles/dreamwhale-v3-agent?version=1'])
    ...
    expect(JSON.stringify(profile)).not.toContain('C:/secret')
  })
  ...
  it('sends a CAS overlay and surfaces revision conflicts', async () => {
    ...
    await expect(client.setOverlay('session-1', ['base', 'dream'], ['style'], 7)).rejects.toMatchObject({ status: 409, currentRevision: 8 })
    expect(body).toEqual({ profileIds: ['base', 'dream'], overlay: { enabledEntries: ['style'], disabledEntries: [] }, expectedRevision: 7 })
  })
