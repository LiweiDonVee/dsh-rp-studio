...
describe('DSH 0.1.2 Remote contract', () => {
  ...
  it('takes session ownership from the durable preset projection', async () => {
    const client = createDshClient({ fetchImpl: async () => response({ items: [
      { sessionId: 's1', projections: { asOfSeq: 8, values: { agentPreset: 'fixture-card' } } },
    ] }) })
    expect(await client.listSessions()).toMatchObject([{ agentPreset: 'fixture-card' }])
  })
    ...
    const client = createDshClient({
      ...
      webSocketFactory: () => {
        ...
        const send = (text: string) => {
          ...
          if (message.type === 'open' && message.endpoint === 'session/follow') queueMicrotask(() => original.onmessage?.({ data: JSON.stringify({ type: 'item', streamId: message.streamId, value: { type: 'snapshot', header: { id: 'cold-rp', agentPreset: 'fixture-card' }, cursor: -1, records: [], hasMore: false, projections: { asOfSeq: -1, values: {} } } }) }))
        }
        return Object.assign(socket, { send }) as unknown as WebSocket
      },
    })
    expect(await client.listSessions()).toMatchObject([{ sessionId: 'cold-rp', agentPreset: 'fixture-card' }])
