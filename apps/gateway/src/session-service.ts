...
import type { Card, PromptPresetSelection, PromptSession, SessionDetail, SessionSummary, StreamEvent } from '@dsh-rp/protocol'
...
export class SessionService implements SessionApi {
  ...
  private async ensureCardWorkspace(card: Card): Promise<DshWorkspace> {
    const home = this.options.dshHome ?? process.env.DSH_HOME ?? join(homedir(), '.dsh')
