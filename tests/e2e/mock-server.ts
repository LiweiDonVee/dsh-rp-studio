...
const cards: Card[] = [
  {
    id: 'fixture-card', title: '测试卡片', description: '合成测试记录', world: '测试环境',
    protagonist: '测试角色', art: 'fixture-card', accent: 'crimson',
    prompt: { coreProfileIds: ['rp-narrative-base', 'fixture-card'], optionalProfileIds: ['fixture-profile'] },
  },
]

function initialState(cardId = 'fixture-card'): PublicGameState {
  if (cardId !== 'fixture-card') throw new Error('测试卡片不可用。')
  return {
    started: true,
    currentDate: '2030-01-01',
    currentTime: '21:40',
    scene: { location: '测试位置', weather: '测试天气' },
    protagonist: {
      name: '测试角色',
    ...
    relationships: [{ id: 'r1', name: '测试关联', status: '测试关系' }],
    faction: [{ id: 'f1', name: '测试组织', status: '测试状态' }],
    inventory: [{ id: 'i1', name: '测试物品', description: '测试说明' }],
    memories: [{ id: 'm1', title: '测试记录', summary: '测试记录 A。' }],
    quests: [{ id: 'q1', title: '测试任务', status: '进行中' }],
    eventLog: [{ id: 'e1', title: '测试事件 A', status: '已记录' }],
    statusLines: ['测试状态 B。'],
    extensions: {},
...
function initialPrompt(cardId = 'fixture-card'): PromptSession {
  if (cardId !== 'fixture-card') throw new Error('测试卡片不可用。')
  return {
    available: true,
    revision: 4,
    ...
    optionalProfiles: [{
      id: 'fixture-profile',
      name: '测试方法组',
      description: 'Synthetic profile metadata',
      version: 1,
      entries: [
        { id: 'fixture-method-a', name: '测试方法 A', slot: 'render-style', group: 'fixture-style', selection: 'single', tags: ['rp'], enabledByDefault: false, renderOnly: true },
        { id: 'fixture-method-b', name: '测试方法 B', slot: 'render-contract', group: 'fixture-pov', selection: 'single', tags: ['rp'], enabledByDefault: false, renderOnly: true },
        { id: 'fixture-method-c', name: '测试方法 C', slot: 'render-style', group: 'fixture-pacing', selection: 'single', tags: ['rp'], enabledByDefault: false, renderOnly: true },
      ],
    }],
    enabledEntryIds: [],
...
function createMockState(): MockState {
  ...
  const session: SessionSummary = {
    id: 'session-1', cardId: cards[0]!.id, title: '测试会话', updatedAt: Date.now(), running: false, blank: false, state,
  }
  return {
    listeners: new Map(),
    timers: new Map(),
    details: new Map([[session.id, {
      ...
      card: cards[0]!,
      messages: [
        { id: 'm1', seq: 1, role: 'gm', text: '测试消息 A。', createdAt: Date.now() - 2_000, status: 'complete' },
        { id: 'm2', seq: 2, role: 'player', text: '测试输入 A。', createdAt: Date.now() - 1_000, status: 'complete' },
      ],
      state,
...
class MockSessionApi implements SessionApi {
  ...
  async prompt(sessionId: string, text: string): Promise<Record<string, unknown>> {
    ...
    this.schedule(tenant, sessionId, 20, () => this.publish(tenant, { type: 'message.delta', sessionId, messageId: `stream-${messageId}`, text: '测试' }))
    this.schedule(tenant, sessionId, 45, () => this.publish(tenant, { type: 'message.delta', sessionId, messageId: `stream-${messageId}`, text: '流片段。' }))
    this.schedule(tenant, sessionId, 750, () => {
      const message: TranscriptMessage = {
        id: messageId, seq: tenant.nextMessage, role: 'gm', text: '测试响应已完成。', createdAt: Date.now(), status: 'complete',
      }
      detail.messages.push(message)
      detail.session.running = false
      detail.state = {
        ...detail.state,
        eventLog: [...detail.state.eventLog, { id: `e${tenant.nextMessage}`, title: '测试事件 B', status: '已更新' }],
        checkpoints: {
