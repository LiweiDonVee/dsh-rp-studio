...
test('empty installation explains user setup without loading content or artwork', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 960 })
  await page.route('**/api/v1/cards', route => route.fulfill({ json: { ok: true, protocolVersion: 1, data: [] } }))
  await page.route('**/api/v1/sessions', route => route.fulfill({ json: { ok: true, protocolVersion: 1, data: [] } }))
  await page.goto('/')
  await expect(page.getByText('未发现可用的 RP 预设')).toBeVisible()
  await expect(page.getByText(/RP Studio 不附带卡片/)).toBeVisible()
  await expect(page.locator('img')).toHaveCount(0)
  await expect(page.getByRole('button', { name: '新建档案' })).toHaveCount(0)
  await page.getByRole('button', { name: '重新加载预设' }).click()
  await expect(page.getByText('未发现可用的 RP 预设')).toBeVisible()
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([])
  await mkdir('test-results/visual', { recursive: true })
  await page.screenshot({ path: 'test-results/visual/studio-empty-1440x960.png' })
})
...
test('desktop narrative workflow streams, rolls back, forks, and controls autoplay', async ({ page }) => {
  ...
  await studio.open()
  await expect(page.locator('.card-artwork img')).toHaveCount(0)
  await studio.send('测试输入 B')
  await expect(page.getByText('测试响应已完成。')).toBeVisible()
  await expect(page.getByText('4 个关键快照')).toBeVisible()

  await page.getByRole('button', { name: '回退上一轮' }).click()
  await expect(page.getByText('测试输入 B')).toHaveCount(0)
  ...
  await page.getByLabel('目标').fill('测试目标')
  await page.getByRole('button', { name: '启动', exact: true }).click()
  await page.getByRole('tab', { name: '任务' }).click()
  await expect(page.getByText('测试目标')).toBeVisible()
  await page.getByRole('button', { name: '管理自动续跑' }).click()
  await page.getByRole('button', { name: '停止', exact: true }).click()
  await expect(page.getByText('测试目标')).toHaveCount(0)

  await page.getByRole('button', { name: '从当前档案创建分支' }).click()
  await expect(page.getByRole('heading', { name: /测试会话 · 分支/ })).toBeVisible()
...
test('creates, resumes, cancels, and switches campaign sessions', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '以测试卡片创建档案' }).click()
  await expect(page.getByRole('heading', { name: '测试卡片' })).toBeVisible()
  await page.reload()
  await expect(page.getByRole('heading', { name: '测试卡片' })).toBeVisible()

  const prompted = page.waitForResponse(response => response.url().endsWith('/messages') && response.request().method() === 'POST')
  await studio.send('测试输入 D')
  await prompted
  await page.getByRole('button', { name: '停止当前回合' }).click()
  await expect(page.getByRole('button', { name: '发送行动' })).toBeVisible()
  await page.waitForTimeout(900)
  await expect(page.getByText('测试响应已完成。')).toHaveCount(0)
  await expect(page.getByText('测试输入 D')).toBeVisible()

  await page.locator('.session-entry').filter({ hasText: '测试会话' }).click()
...
test('keeps optional narrative methods empty by default and applies them next turn', async ({ page }) => {
  ...
  await expect(page.getByRole('checkbox', { name: '测试方法 A' })).not.toBeChecked()

  await page.getByRole('checkbox', { name: '测试方法组全部方法' }).check()
  ...
  await page.reload()
  await page.getByRole('tab', { name: '方法' }).click()
  await expect(page.getByRole('checkbox', { name: '测试方法 A' })).toBeChecked()
  await studio.expectNoHorizontalOverflow()
})
...
test('mobile sheets preserve the narrative as the primary surface', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: '世界与会话' }).getByText('测试卡片')).toBeVisible()
