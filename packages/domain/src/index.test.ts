  ...
  it('normalizes public collections and rejects every private canary path', () => {
    const state = projectPublicState({
      ...
      game: {
        ...
        scene: {
          location: '测试地点',
          currentBeat: { title: '测试事件', objectives: ['测试目标'] },
        },
        protagonist: {
          id: 'hero',
          name: '测试主角',
        ...
        relationships: {
          ally: { id: 'ally', name: '测试关联', trust: 78 },
          hidden: { id: 'spy', name: SECRET, visibility: 'hidden-canonical' },
        },
        faction: [{ id: 'team', name: '测试组织' }],
        inventory: [{ id: 'flashlight', name: '测试物品' }],
        memories: {
          public: [{ id: 'm1', title: '测试记录 A' }],
          protagonist: [{ id: 'm2', title: '测试记录 B' }],
          private: [{ id: 'm3', title: SECRET }],
        },
        quests: { escape: { id: 'escape', title: '测试任务', status: 'active' } },
        eventLog: [{ kind: 'scene', summary: '测试事件说明' }],
        driver: { armed: false, objective: '', maxRounds: 8 },
        statusLines: ['2032-04-11 18:30', '测试地点'],
        economy: { credits: 80 },
        researchNotebook: { public: { title: '测试扩展' } },
    ...
    expect(state.relationships).toEqual([{ id: 'ally', name: '测试关联', trust: 78 }])
